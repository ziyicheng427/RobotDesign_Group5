...
class Explore(Node):
    def __init__(self):
        ...
        self.manipulator_pose_pub = self.create_publisher(
            Point, "/px150/joint_states", 10)
        ...
        self.obj = None
        self.manipulator = Point()

    def camera_subscriber_callback(self, msg: Pose):
       
        """
        Callback function for processing Pose messages received from the /object_position topic.
        """
        self.obj = msg.Point
    ...
    def determine_frontier(self, index):
        ...
        start_posx = int(index[0] - grid_length / 2)
        start_posy = int(index[1] - grid_length / 2)
        ...
        for i in range(grid_length):
            for j in range(grid_length):
                ...
                #if child_position_row > len(self.original_occupancy_grid_data) and child_position_column > len(self.original_occupancy_grid_data[0]):
                 #   print("higher")
                  #  continue
                child_position = (child_position_row,
                                  child_position_column)

                #elf.get_logger().info(f"child_position: {child_position}")
                ...
                if self.original_occupancy_grid_data[
                    ...
                    if num_walls > wall_threshold:
                        return False
        self.get_logger().info(f"new point: {child_position}")
        if num_frontiers > 2:
            print("FRONIER NUMBER",num_frontiers)
            return True
        else:
            print("FRONIER NUMBER",num_frontiers)
            return False
    ...
    def timer_callback(self):
        ...
        if self.state == State.FIND_ROBOT:
            ...
            self.robot_location = self.map_coords_to_occupancy_grid(
                robot_map_point)
            self.goal_location.append(self.robot_location)
            ...
            for cell in self.fringe:
                ...
                for modifier in self.child_positions:
                    ...
                    if child_position not in self.visited and child_position not in self.fringe:
                        print(child_position_row, child_position_column)

                        # Check if indices are within bounds
                        if  child_position_row < len(self.original_occupancy_grid_data) and \
                                child_position_column < len(self.original_occupancy_grid_data[0]):
                            if self.original_occupancy_grid_data[child_position_row][child_position_column] == -1 and far_away > 1.5:
                                if not is_frontier:
                                    is_frontier = self.determine_frontier(child_position)
                            elif self.original_occupancy_grid_data[child_position_row][child_position_column] == 0:
                                # fringe is a list of cells where I care about their children
                                marker = self.get_marker(self.occupancy_grid_to_map_coords(child_position))
                                self.marker_pub.publish(marker)
                                self.fringe.append(child_position)
                            elif self.original_occupancy_grid_data[child_position_row][child_position_column] == 100:
                                self.visited.append(child_position)
                                self.blocks.append(child_position)
                            else:
                                self.fringe.append(child_position)
                        else:
                            print("Indices out of bounds! Moving to the next point.")
                            continue  # Skip the rest of the current iteration and move to the next point
            ...
            if self.obj:
                ...
                self.state = State.GO_TO_OBJECT
           
            if len(self.visited) >= len(self.original_occupancy_grid_data) * len(self.original_occupancy_grid_data[0]):
                print("Exploration completed!")
                self.state = State.RETURN_HOME  
             

        elif self.state == State.GO_TO_OBJECT:
            self.gotoobj = self.occupancy_grid_to_map_coords(
                self.obj)
            self.gotoobj.header = Header(
                stamp=self.get_clock().now().to_msg())
            self.gotoobj.header.frame_id = "map"

            self.goal_pose_pub.publish(self.gotoobj)
            self.manipulator_pose_pub.publish(self.manipulator)
            self.state = State.WAITING



        elif self.state == State.RETURN_HOME:
            print("GOAL:",self.goal_location[0]," START: ",self.start_location)
            self.goback = self.occupancy_grid_to_map_coords(
                self.goal_location[0])
            self.goback.header = Header(
                stamp=self.get_clock().now().to_msg())
            self.goback.header.frame_id = "map"

            self.goal_pose_pub.publish(self.goback)
            self.state = State.WAITING
